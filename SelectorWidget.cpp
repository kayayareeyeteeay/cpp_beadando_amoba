...
void SelectorWidget::draw() {
    ...
    gout << move_to(_x+_size_x-20, _y+_size_y/2+5) << text("V");

    if (_opened) {
        for (int i = 0; i < _max_visible && (i + _scroll_offset) < _options.size(); i++) {
            int cur_idx = i + _scroll_offset;
            int cur_y = _y + _size_y + (i * _size_y);
            gout << move_to(_x, cur_y) << color(255,255,255) << box(_size_x, _size_y);
            if (cur_idx == _selected) gout << move_to(_x+1, cur_y+1) << color(100,100,255) << box(_size_x-2, _size_y-2);
            else gout << move_to(_x+1, cur_y+1) << color(60,60,60) << box(_size_x-2, _size_y-2);
...
void SelectorWidget::handle(event ev) {
        ...
        if (ev.pos_x > _x && ev.pos_x < _x + _size_x && ev.pos_y > _y && ev.pos_y < _y + _size_y) {
            ...
            for (int i = 0; i < _max_visible; i++) {
                ...
                if (ev.pos_x > _x && ev.pos_x < _x + _size_x && ev.pos_y > elem_y && ev.pos_y < elem_y + _size_y) {
                    ...
                    if (kivalasztott_index < _options.size()) {
    ...
    if (_opened && ev.type == ev_mouse) {
        ...
        if (ev.button == btn_wheeldown && _scroll_offset + _max_visible < _options.size()) _scroll_offset++;
    }
}
bool SelectorWidget::is_selected(int mx, int my) {
    if (!_opened) {
        ...
        int teljes_magassag = _size_y + (_max_visible * _size_y);
